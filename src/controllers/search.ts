...
const getSearchResults = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const searchPromises = [
      ...
      History.find({
        $or: [{ text: { $regex: searchQuery, $options: 'i' } }, { author: { $regex: searchQuery, $options: 'i' } }],
      }),
      ...
      News.find({
        $or: [{ title: { $regex: searchQuery, $options: 'i' } }, { newsText: { $regex: searchQuery, $options: 'i' } }],
      }),
    ...
    const [articles, audios, ourHistory, members, news, projects, reports, scores, videos] = await Promise.all(
      searchPromises,
    );
